...
function mergeEntityMarkdown(existing: string, incoming: string): string {
...
async function upsertEntityFile(
  ...
  try {
    ...
    const hasCurrentNoteLink = existing.includes(noteLink);
    const merged = mergeEntityMarkdown(existing, content);

    if (hasCurrentNoteLink && merged === existing) {
      return "updated";
    }
